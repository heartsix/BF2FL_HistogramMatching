...
# 子图2：残差分布直方图
plt.subplot(1, 2, 2)
plt.hist(residual_gray.flatten(),range=(0,25), bins=26, color='gray', alpha=0.7)
...
# 计算每个通道的残差
residual_rgb = cv2.absdiff(bf_img_rgb, fl_img_rgb)
...
for i in range(3):
    ...
    # 绘制直方图
    plt.hist(channel_data, bins=51, range=(0, 50), color=colors[i][0].lower(), alpha=0.7)
